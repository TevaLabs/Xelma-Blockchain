...
fn test_heartbeat_override_emits_event() {
    ...
    let events = env.events().all();
    // The override is consumed during resolve_round (silent consume, no dedicated event).
    // Verify the override was actually consumed by checking it's no longer armed.
    assert!(!client.get_hb_override_armed(), "override must be consumed after resolve_round");
}
...
fn test_arm_heartbeat_override_emits_event() {
    ...
    // Verify the override is armed (arm_hb_override only emits on rejection, not on success).
    assert!(client.get_hb_override_armed(), "override must be armed after arm_hb_override");
}

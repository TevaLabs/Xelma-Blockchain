...
use super::config_helpers::{apply_oracle_max_deviation_bps, apply_oracle_stale_threshold, apply_windows};
...
fn test_resolve_round_stale_timestamp() {
    ...
    // Submit payload with timestamp 600 — outside the round-relative economic
    // window (start=0, end_ledger=12 -> end_estimate=60, default skew=300 ->
    // window=[0,360]). The round-relative window check supersedes the old
    // absolute-freshness (300s) check for this path (see resolve_round).
    let payload = OraclePayload {
        price: 1_5000000,
        timestamp: 600,
...
fn test_heartbeat_gate_strict_off_allows_settlement_even_when_stale() {
    ...
    client.initialize(&admin, &oracle);
    // Widen the round's run window so the round-relative oracle timestamp
    // check (Issue: oracle-round-relative-timestamp-window) doesn't reject
    // the payload before the heartbeat gate is even reached — the default
    // 12-ledger window is far shorter than the 3600s stale threshold below.
    apply_windows(&env, &client, 6, 800);
    client.create_round(&1_0000000, &None);
    ...
    // Advance past end_ledger (widened to 800 ledgers above), but strict
    // mode is OFF (default)
    env.ledger().with_mut(|li| {
        li.sequence_number = 800;
...
fn test_heartbeat_gate_strict_on_blocks_when_stale_past_grace() {
    ...
    client.initialize(&admin, &oracle);
    client.set_hb_strict_mode(&true);
    // Widen the round's run window past the 3600s stale threshold below —
    // see the comment in test_heartbeat_gate_strict_off_allows_settlement_even_when_stale.
    apply_windows(&env, &client, 6, 800);
    client.create_round(&1_0000000, &None);
...
fn test_heartbeat_gate_override_bypasses_block_and_emits_event() {
    ...
    // Verify override event emitted. Must be read before any further client
    // call — the test host's event buffer is scoped per top-level
    // invocation, so an intervening call (e.g. get_active_round) would wipe
    // events published during resolve_round.
    let events = env.events().all();
    let override_event = events.iter().find(|e| {
        let (_contract, topics, _data) = e;
        topics.len() == 2
...
fn test_heartbeat_gate_grace_period_allows_stale_within_grace() {
    ...
    // Widen the round's run window past the 3600s+600s target below — see
    // the comment in test_heartbeat_gate_strict_off_allows_settlement_even_when_stale.
    apply_windows(&env, &client, 6, 900);
    ...
    // and past end_ledger (widened to 900 ledgers above)
    env.ledger().with_mut(|li| {
